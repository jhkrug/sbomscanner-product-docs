...
|`enabled`
a|Enable or disable workload scanning. See xref:#_disabling-workload-scanning[Disabling workload scanning] for details.
|`true`

|`artifactsNamespace`
a|Namespace where managed `Registry`, `ScanJob`, `Image`, `SBOM`, and `VulnerabilityReport` resources are created. `WorkloadScanReport` resources are always created in the workload's namespace regardless of this setting. If omitted or empty, managed artifacts are created in the workload's namespace. See xref:#_multi-tenancy-setup[Multi-tenancy setup] for details.
...
|`platforms`
a|Which platforms to scan for each discovered image. See xref:#_scanning-multiple-platforms[Scanning multiple platforms].
|All platforms
...
[source,bash]
----
kubectl get workloadscanreports -n evantage -o yaml | yq '.items[] | select(.metadata.ownerReferences[] | .kind == "Deployment" and .name == "evantage-nettools")'
----
